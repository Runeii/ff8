...
const useMethod = (
  ...
  remoteExecutionRequests: RemoteExecutionRequest[],
  setRemoteExecutionRequests: (requests: RemoteExecutionRequest[]) => void,
  isActive: boolean,
  ...
  const activeMethod = useMemo(() => {
    ...
    console.log('calculating active method', remoteExecutionRequests)
    
    if (!hasCompletedConstructor) {
      return constructor
    }

    if (remoteExecutionRequests.length > 0) {
      const request = remoteExecutionRequests[0];
      const method = methods.find(method => method.methodId === request.methodId)!; // we pre validate this exists before adding to queue
      return method;
    }
  ...
  }, [activeMethodId, hasCompletedConstructor, isActive, isHalted, previousActiveMethodName, remoteExecutionRequests, script.methods]);
  ...
  }, [activeMethod, hasCompletedConstructor]);

  const handleCompleteRun = useCallback(() => {
    if (!activeMethod) {
      console.log('No active method') 
      return;
    }

    if (remoteExecutionRequests.length > 0) {
      console.log('WAs remote execution request')
      const request = remoteExecutionRequests[0];
      const method = script.methods.find(method => method.methodId === request.methodId)!; // we pre validate this exists before adding to queue
      
      if (method.methodId === activeMethod.methodId) {
        console.log('Removing remote execution request')
        setRemoteExecutionRequests(remoteExecutionRequests.slice(1));
        document.dispatchEvent(new CustomEvent('scriptFinished', { detail: { key: request.key } }));
      }
    }
    ...
    const isDoorOpenOrCloseState = script.type === 'door' && (activeMethod.methodId === 'open' || activeMethod.methodId === 'close');
  ...
  }, [activeMethod, remoteExecutionRequests, script.methods, script.type, setActiveMethodId, setRemoteExecutionRequests, useScriptStateStore]);
  ...
  useEffect(() => {
    ...
    execute();
  }, [activeMethod, animationController, currentOpcodeIndex, handleCompleteRun, hasCompletedConstructor, isLooping, isUnused, scene, script, useScriptStateStore]);
